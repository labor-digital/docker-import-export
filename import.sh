...
GRANT EXECUTE, SHOW VIEW, ALTER, ALTER ROUTINE, CREATE, CREATE ROUTINE, CREATE TEMPORARY TABLES, CREATE VIEW, DELETE, DROP, EVENT, INDEX, REFERENCES, TRIGGER, LOCK TABLES ON ${APP_MYSQL_DATABASE}.* TO ${APP_MYSQL_USER}@'%';
FLUSH PRIVILEGES;
"
...
GRANT EXECUTE, SHOW VIEW, ALTER, ALTER ROUTINE, CREATE, CREATE ROUTINE, CREATE TEMPORARY TABLES, CREATE VIEW, DELETE, DROP, EVENT, INDEX, REFERENCES, TRIGGER, LOCK TABLES ON ${!APP_MYSQL_DATABASE_IDX_NAME}.* TO ${APP_MYSQL_USER}@'%';
FLUSH PRIVILEGES;
"
			mysql --host="${APP_MYSQL_HOST}" --user='root' --password="${MYSQL_ROOT_PASSWORD}" -f <<< $sqlinitstr
